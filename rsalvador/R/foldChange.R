# Nov 3, 2020, tackling fold change for the LD model

LD.fold.score.info = function(data.x, data.y, Nx, Ny, b0, b1, phi.x=1.0, phi.y=1.0) {
  ...
  px = prob.LD(mx, phi.x, nx)
 
  py = prob.LD(my, phi.y, ny)
 
  hx = apply(as.array(1:nx), 1, function(x) phi.x ^ (x-1) * (1 / x - phi.x / (x + 1)) )

  hx = append(hx,-1, after = 0)

  hy = apply(as.array(1:ny), 1, function(x) phi.y ^ (x-1) * (1 / x - phi.y / (x + 1)) )

  hy = append(hy, -1, after = 0)

  px1 = seq.convolute(px, hx)

  px2 = seq.convolute(px1, hx)

  py1 = seq.convolute(py, hy)

  py2 = seq.convolute(py1, hy)

  F1 = sum((px1/px)[data.x + 1])

  F12 = sum(((px1/px)^2)[data.x + 1])

  F2 = sum((px2/px)[data.x + 1])

  F3 = sum((py1/py)[data.y + 1])

  F32 = sum(((py1/py)^2)[data.y + 1])

  F4=sum((py2 / py)[data.y + 1])
...
newton.foldchange.LD = function (x, y, Nx, Ny, phi.x=1.0, phi.y=1.0, tol = 1e-08, init.base = 1.5e-8, init.fold = 1.6, 
  ...
  srinfo = LD.fold.score.info(x, y, Nx, Ny, beta0[1], beta0[2], phi.x = phi.x, phi.y = phi.y)
...
fold.venzon.b1 = function(x, y, Nx, Ny, b0.hat, b1.hat, alpha=0.05, phi.x=1.0, phi.y=1.0) {

  srinfo = LD.fold.score.info(x, y, Nx, Ny, b0.hat, b1.hat, phi.x = phi.x, phi.y = phi.y)
...
fold.loglik.LD = function(x, y, Nx, Ny, b0, b1, phi.x= 1.0, phi.y = 1.0) {

  mx = Nx*exp(b0)

  my = Ny * exp(b0 + b1)

  probx = prob.LD(m = mx, phi = phi.x, k = max(x))

  proby = prob.LD(m = my, phi = phi.y, k = max(y))
...
confint.foldchange.LD=function(x, y, Nx, Ny, phi.x = 1.0, phi.y = 1.0, alpha = 0.05, init.base=1e-8, init.fold=1.6,
 
      init.low.base = -9, init.up.base = -9, init.low.fold = -9, init.up.fold = -9, max.iter=30, tol=1e-9,
  ...
  mle = newton.foldchange.LD(x, y, Nx, Ny, phi.x = phi.x, phi.y = phi.y, init.base = init.base, init.fold = init.fold, 
  ...
  if ( (init.low.base < 0) || (init.low.fold < 0) || (init.up.base < 0) || (init.up.base < 0) )
    ...
    {all.inits = fold.venzon.b1(x, y, Nx, Ny, b0.hat, b1.hat, phi.x = phi.x, phi.y = phi.y, alpha = alpha) }
...
### The matrix and vector in equation 37 of Zheng (Math. Biosci, 2015) 
  ...
  srinfo = LD.fold.score.info(x, y, Nx, Ny, b00, b10, phi.x = phi.x, phi.y = phi.y)

  zheng37mat = matrix(c(-srinfo[1], -srinfo[2], srinfo[3], srinfo[4]), ncol=2, byrow=TRUE)

  like = fold.loglik.LD(x, y, Nx, Ny, b00, b10, phi.x = phi.x, phi.y = phi.y)

  la = fold.loglik.LD(x, y, Nx, Ny, b0.hat, b1.hat, phi.x = phi.x, phi.y = phi.y) - 0.5 * qchisq(1-alpha, df = 1) # Nov 9, 2020
  ...
  ### updating .........
    ...
    srinfo = LD.fold.score.info(x, y, Nx, Ny, b01, b11, phi.x = phi.x, phi.y = phi.y)

    zheng37mat = matrix(c(-srinfo[1], -srinfo[2], srinfo[3], srinfo[4]), ncol = 2, byrow = TRUE)

    like=fold.loglik.LD(x, y, Nx, Ny, b01, b11, phi.x = phi.x, phi.y = phi.y)
...
### The matrix and vector in equation 37 of Zheng (Math. Biosci, 2015) 
  ...
  srinfo = LD.fold.score.info(x, y, Nx, Ny, b00, b10, phi.x = phi.x, phi.y = phi.y) ### Nov 9, 2020

  zheng37mat = matrix(c(-srinfo[1], -srinfo[2], srinfo[3], srinfo[4]), ncol = 2, byrow = TRUE)

  like = fold.loglik.LD(x, y, Nx, Ny, b00, b10, phi.x = phi.x, phi.y = phi.y)

  la = fold.loglik.LD(x, y, Nx, Ny, b0.hat, b1.hat, phi.x = phi.x, phi.y = phi.y) - 0.5 * qchisq(1 - alpha, df = 1)
...
########### Bayes ###########
# -------- Bayesian approach to fold change, based on existing mcmc.MK.test, Nov 7, 2020 -------------

bayes.foldchange.MK=function(y1, y2, N1, N2, w1 = 1, w2 = 1, s0 = 0.4, s1 = 0.5, init.base = 1e-8, init.fold = 1.6,
       v0 = 100, v1 = 100, iter = 1100, burn = 100, thin = 1, alpha = 0.05, short.out = TRUE, show.simu = FALSE) {
  ...
  simu.chain = mcmc.MK.test(y1, y2, N1, N2, w1 = w1, w2 = w2, s0 = s0, s1 = s1, mu0 = mu0, mu1 = mu1,
...
boot.MK.one = function(y1, y2, N1, N2, w1 = 1, w2 = 1, init.m1 = 0, init.m2 = 0) {
  ...
  mu1 = newton.MK(z1, w = w1, init.m = init.m1) / N1

  mu2 = newton.MK(z2, w = w2, init.m = init.m2) / N2
...
boot.foldchange.MK = function(y1, y2, N1, N2, w1 = 1, w2 = 1, alpha = 0.05, init.m1 = 0, init.m2 = 0, 
  ...
  for (i in 1:nboot) rates[i] = boot.MK.one(y1, y2, N1, N2, w1 = w1, w2 = w2, init.m1 = init.m1, init.m2 = init.m2)
